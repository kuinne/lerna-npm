    ...
    Metalsmith(__dirname)
      ...
      .use((files, metalsmith, done) => {
        ...
        Object.keys(files).forEach((fileName) => {
          // 需先转换为字符串
          const fileContentString = files[fileName].contents.toString();
          // 重写文件内容
          files[fileName].contents = Buffer.from(
            // 使用定义的metaData取代模板变量
            handlebars.compile(fileContentString)(metalsmith.metadata())
          );
        });
...
entry = ({ sModule, sDescription, sName }) => {
  ...
  pullLocalTemp(foldPath, sModule, sDescription, sName)
    .then(() => {
      console.log(
        chalk.green(
          `[SUCCESS] Congratulations! Then "${sModule}" create successfully!`
        )
      );
    })
    .catch(() => {
...
inquirer
  ...
  .catch(() => [console.log(chalk.red(`[ERROR] Inquirer error`))]);
